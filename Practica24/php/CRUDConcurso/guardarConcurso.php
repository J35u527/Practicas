<?php
	...
	$query = "UPDATE `concurso` SET `nombreConcurso` = '$nomConcurso',`hashtag` = '$hashtag',`dificultad` = $dificultad,`categoria` = $categoria,`fechaDeAlta` = '$fechaAlta',`fechaDeInicio` = '$fechaInicio,`descripcion` = '$descripConcurso',`fechaDeFin` = '$fechaFin',`status` = 1,`motivos` = 'falta revisar',`usuarioGanador` = 1,`usuarioOrganizador` = $idUsuario WHERE `concurso`.`categoria` = $categoria AND `concurso`.`usuarioGanador` = 1 AND `concurso`.`usuarioOrganizador` = $idUsuario;
